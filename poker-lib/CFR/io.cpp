...
bool Reader::Refresh(void) {
  ...
  if (overflow_size_ > 0) {
    memcpy(buf_, overflow_, overflow_size_);
  }
  buf_ptr_ = buf_;
    
  unsigned char *read_into = buf_ + overflow_size_;
...
#if 0
// This assumes buf_ is empty
bool Reader::Skip(long long int num_to_skip) {
  if (num_to_skip > remaining_) return false;
  while (num_to_skip > kBufSize) {
    int ret;
    if ((ret = read(fd_, buf_, kBufSize)) != kBufSize) {
      fprintf(stderr, "Read returned %i not %i\n", ret, kBufSize);
      exit(-1);
    }
    num_to_skip -= kBufSize;
  }
  if (num_to_skip > 0) {
    int ret;
    if ((ret = read(fd_, buf_, num_to_skip)) != num_to_skip) {
      fprintf(stderr, "Read returned %i not %i\n", ret, num_to_skip);
      exit(-1);
    }
  }
...
unsigned char *Reader::ReadBytesOrDie(int *num_bytes) {
  *num_bytes = ReadIntOrDie();
...
void Writer::Flush(void) {
  if (buf_ptr_ > buf_) {
    int written = write(fd_, buf_, (int)(buf_ptr_ - buf_));
    if (written != (int)(buf_ptr_ - buf_)) {
      fprintf(stderr,
	      "Error in flush: tried to write %i, return of %i; fd %i\n",
	      (int)(buf_ptr_ - buf_), written, fd_);
      exit(-1);
    }
  }
  buf_ptr_ = buf_;
