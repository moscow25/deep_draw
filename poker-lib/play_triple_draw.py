    ...
    def choose_action(self, actions, round, bets_this_round = 0, 
        ...
        if bets_layer and self.use_learning_action_model:
            ...
            if USE_ACTION_PERCENTAGE and np.random.rand() <= ACTION_PERCENTAGE_CHOICE_RATE:
                ...
                probabilities = [action_tuple[0] for action_tuple in action_percentge]
                # Defensively, fix possible issues with sum(prob) != 1.0
                remainder = 1.0 - np.asarray(probabilities).sum()
